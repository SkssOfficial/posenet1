...
function draw() {
    ...
    if (waitTime > -1) {
        ...
        textAlign(LEFT, TOP);
        // fill(255, 0, 0);
        text("Stage: " + String(parseInt(stage) + 1), 0, 0);
        text("Count: " + stageResult[0] + ", " + stageResult[1], 0, 80);
        text("Mark: " + Math.max(stageResult[0], stageResult[1]) + '/8', 0, 160);
        text("Time Left: " + nextStageTime, 0, 240)
    ...
    for (let i = 0; i < poses.length; i++) {
        ...
        switch (String(stage)) {
            ...
            case '2':
                ...
                if (rightKnee.y > leftKnee.y + poseOffset && leftShoulder.y + poseOffset > leftElbow.y  ) {
                    stageResult[0] += 1
                } else if (leftKnee.y > rightKnee.y + poseOffset && rightShoulder.y + poseOffset > rightElbow.y  ) {
                    stageResult[1] += 1
                }
                break;
           // case '4':
